...
LangChain의 `create_react_agent`(ReAct 패턴), `create_tool_calling_agent`(OpenAI Function Calling 패턴)부터 시작해서, LangGraph의 `StateGraph` 기반 에이전트와 `langgraph.prebuilt.create_react_agent`까지 전부 시도해봤습니다. 단순한 데모 — "검색해서 요약해줘" 수준 — 에서는 잘 동작합니다.
