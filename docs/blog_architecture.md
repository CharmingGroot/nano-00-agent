...
- **도구 호출 결과가 반복 실행되며 수만 토큰까지 누적**되는 상황을 컨텍스트 오버플로우 없이 처리
- **태스크 중간에 사람이 확인**(HITL)하고 승인해야 다음 단계로 진행
- **30B급 sLLM**으로 이 모든 걸 해야 함 (GPU 예산 한정)
...
**3. 미들웨어는 있지만, 커스텀이 어렵습니다.**
LangChain 1.0의 `create_agent`는 미들웨어 시스템을 도입했고, 성능도 꽤 좋습니다. 하지만 "모든 LLM 호출 전에 토큰을 세고, 임계값을 넘으면 구조화된 상태 JSON으로 압축하고, 도구 결과는 Goal 기준으로 필터링하고, 매 스텝 후에 진행률을 평가하고, 포인터를 DB에서 선택적으로 resolve한다" — 이 수준의 **세밀한 제어를 프레임워크의 미들웨어 인터페이스 안에서** 구현하려면 한계에 부딪힙니다. 결국 프레임워크의 내부 구조를 우회하거나 몽키패칭하게 되는데, 그러면 프레임워크를 쓰는 의미가 사라집니다.
...
**3. 단일 요청의 토큰이 매우 큽니다.**
웹 검색 한 번의 결과는 1,000~2,000 토큰 수준이지만, 에이전트는 요구를 만족할 때까지 도구를 반복 호출합니다. 검색 → 부족 → 재검색 → 보완 검색을 거치면 수만 토큰이 누적됩니다. 문서 RAG도 마찬가지로 청크를 추가로 가져오며 쌓입니다. 이렇게 **반복 호출의 누적**이 컨텍스트를 빠르게 소진합니다.
...
| 소스 | Raw 토큰 | 압축 후 |
|------|---------|--------|
| 웹 검색 (1회 1~2K, 반복 누적) | 10,000~30,000 | 3,500 |
| 지식 RAG (청크 추가 반복) | 8,000~20,000 | 3,200 |
| 데이터셋 조회 (수백 행) | 15,000~25,000 | 4,000 |
...
아직 완벽하지 않습니다. 개선할 부분도 많고, 실전에서 부딪힐 엣지 케이스도 있을 겁니다. 더 나은 구조나 아키텍처를 발견하면 계속 공유하겠습니다.
